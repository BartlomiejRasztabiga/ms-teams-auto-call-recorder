...
def checkAndJoinMeeting():
    ...
    if elem.get_attribute('aria-pressed') == 'true': # turn off microphone
        elem.click()
    wait_and_find_element_by_xpath('//button[.="Dołącz teraz"]', timeOutDelay).click() # join meeting
...
def init():
    ...
    wait_and_find_ele_by_link_text('Zamiast tego użyj aplikacji internetowej', timeOutDelay).click() # click use the web app instead link  
    while wait_and_find_element_by_xpath('//button[@title="Zmień widok kalendarza"]', timeOutDelay) == None: #   wait for calendar tab to completely load
        sleep(5)
    while wait_and_find_element_by_xpath('//button[@title="Zmień widok kalendarza"]', timeOutDelay).get_attribute('name') != "Tydzień roboczy": # change calender work-week view to week view
        wait_and_find_element_by_xpath('//button[@title="Zmień widok kalendarza"]', timeOutDelay).click()
        wait_and_find_element_by_xpath('//button[@name="Tydzień roboczy"]', timeOutDelay).click()
